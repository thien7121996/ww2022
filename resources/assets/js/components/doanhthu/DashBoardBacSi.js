...
class DashBoardBacSi extends Component {
  constructor (props) {
    super(props)
    this.state = {
      listdoanhthu: [],
...
axios.get('/index.php/api/laytienkhtongtheotungbacsitrongthang/'+month+'?nam='+new Date().getFullYear()).then(response => {
...
axios.get('/index.php/api/doctor').then(response => {
 ...
 handleBoLoc(e){
	 ...
	 axios.get('/index.php/api/laytienkhtongtheotungbacsitrongthang/'+thang+'?nam='+nam).then(response => {
    if(response.status==200)
	{
		this.setState({
      listdoanhthu: response.data
  ...
  render() {
    ...
    return (
      ...
      <div className="row App" id="lichbooking">
        ...
        <h3 className="box-title">DOANH THU TỪNG BÁC SĨ TRONG THÁNG <span id="thangdangxem">{month}</span>
  </h3>
  <div className="row">
 ...
 var html="";
  ...
  {listdoanhthu.map((dv, index) => (
  <div className="col-md-12 col-xs-12 m-b-20">
   <h3 className="box-title">DOANH THU BÁC SĨ CỦA CHI NHÁNH {dv.tenchinhanh}</h3>
    <PieChart
      id="pie"
