...
class DashBoardTungNgay extends Component {
  constructor (props) {
    super(props)
    this.state = {
      listdoanhthu: [],
...
axios.get('/index.php/api/laytienkhtongtheotungngaytrongthang/'+month+'?nam='+new Date().getFullYear()).then(response => {
 ...
 handleBoLoc(e){
	 ...
	 axios.get('/index.php/api/laytienkhtongtheotungngaytrongthang/'+thang+'?nam='+nam).then(response => {
  ...
  render() {
    ...
    return (
      ...
      <div className="row App" id="lichbooking">
        ...
        <h3 className="box-title">DOANH THU TỪNG NGÀY TRONG THÁNG <span id="thangdangxem">{month}</span>
  </h3>
 ...
 var html="";
  ...
  {listdoanhthu.map((dv, index) => (
  <div className="col-md-12 col-xs-12 m-b-20">
   <h3 className="box-title">DOANH THU TỪNG NGÀY CỦA CHI NHÁNH {dv.tenchinhanh}</h3>
   <Chart
        id="chart"
...
export default DashBoardTungNgay;