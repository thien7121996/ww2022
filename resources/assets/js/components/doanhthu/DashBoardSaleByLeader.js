...
class DashBoardSaleByLeader extends Component {
  constructor (props) {
    super(props)
    this.state = {
      listdoanhthu: [],
...
axios.get('/index.php/api/laytienkhtongtheotungsaletrongthangleader/'+month+'?iduser='+localStorage.getItem('userid')+'&nam='+new Date().getFullYear()).then(response => {
...
axios.get('/index.php/api/danhsachtaikhoanrolesalebyleader/'+localStorage.getItem('userid')).then(response => {
    this.setState({
 ...
 handleBoLoc(e){
	 ...
	 axios.get('/index.php/api/laytienkhtongtheotungsaletrongthangleader/'+thang+'?iduser='+localStorage.getItem('userid')+'&nam='+nam).then(response => {
   if(response.status==200)
	{
		this.setState({
      listdoanhthu: response.data
  ...
  render() {
    ...
    return (
      ...
      <div className="row App" id="lichbooking">
        ...
        <h3 className="box-title">DOANH THU TỪNG SALE TRONG THÁNG <span id="thangdangxem">{month}</span>
  </h3>
  <div className="row">
 ...
 var html="";
          for (var i = 0; i <= listgioithieu.length; i++) {
			  ...
			  if(i<listgioithieu.length)
			  {
				  ...
				  html=html+"<td>"+listgioithieu[i]["name"]+"</td>";
				  for (var j = 0; j<listdoanhthu.length; j++) {
					  
					html=html+"<td>"+listdoanhthu[j].doanhthu[i].tongthanhtien.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+"</td>";                 
				  }
			  ...
			  else
			  {
				  ...
				  for (var j = 0; j<listdoanhthu.length; j++) {
					...
					html=html+"<td>"+listdoanhthu[j].tongtienchinhanh.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+"</td>";                 
				  }
			  }
			  html=html+"</tr>";
  ...
  {listdoanhthu.map((dv, index) => (
 <div className="col-md-12 col-xs-12 m-b-20">
   <h3 className="box-title">DOANH THU TELE SALE CỦA CHI NHÁNH {dv.tenchinhanh}</h3>
    <PieChart
      id="pie"
      dataSource={dv.doanhthu}
      palette="Bright"
    
    >
       <Series
          argumentField="tensale"
          valueField="tongthanhtien"
        >
      ...
      <Tooltip enabled={true} format="fixedPoint" customizeTooltip={customizeTooltip} />
    </PieChart>
  </div>
...
export default DashBoardSaleByLeader;