...
class DashBoardTeleSale extends Component {
  constructor (props) {
    super(props)
    this.state = {
      listdoanhthu: [],
...
axios.get('/index.php/api/laytienkhtongtheotungsaletrongthang/'+month+'?nam='+new Date().getFullYear()).then(response => {
...
axios.get('/index.php/api/danhsachtaikhoansale').then(response => {
 ...
 handleBoLoc(e){
	 ...
	 axios.get('/index.php/api/laytienkhtongtheotungsaletrongthang/'+thang+'?nam='+nam).then(response => {
   if(response.status==200)
	{
		this.setState({
      listdoanhthu: response.data
...
export default DashBoardTeleSale;