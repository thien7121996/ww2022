...
class DashBoardNguon extends Component {
  constructor (props) {
    super(props)
    this.state = {
      listdoanhthu: [],
...
axios.get('/index.php/api/laytienkhtongtheotungnguontrongthang/'+month+'?nam='+new Date().getFullYear()).then(response => {
...
axios.get('/index.php/api/nguongioithieu').then(response => {
 ...
 handleBoLoc(e){
	 ...
	 axios.get('/index.php/api/laytienkhtongtheotungnguontrongthang/'+thang+'?nam='+nam).then(response => {
    if(response.status==200)
	{
		this.setState({
      listdoanhthu: response.data
  ...
  render() {
    ...
    return (
      ...
      <div className="row App" id="lichbooking">
        ...
        <h3 className="box-title">DOANH THU TỪNG NGUỒN TRONG THÁNG <span id="thangdangxem">{month}</span>
  </h3>
  <div className="row">
 ...
 var html="";
          for (var i = 0; i <= listgioithieu.length; i++) {
			  ...
			  if(i<listgioithieu.length)
			  {
				  ...
				  html=html+"<td>"+listgioithieu[i]["nguon"]+"</td>";
  ...
  {listdoanhthu.map((dv, index) => (
  <div className="col-md-12 col-xs-12 m-b-20">
   <h3 className="box-title">DOANH THU NGUỒN CỦA CHI NHÁNH {dv.tenchinhanh}</h3>
    <PieChart
      id="pie"
...
function customizeTooltip(pointInfo) {
  return {
    text: `${pointInfo.argumentText}<br/>${pointInfo.valueText} (${pointInfo.percentText})`,
  };
}
