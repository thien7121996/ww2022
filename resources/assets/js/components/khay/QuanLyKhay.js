...
class QuanLyKhay extends Component {
  constructor (props) {
    super(props)
    this.state = {
      listkhay: '',
...
axios.get('/index.php/api/danhsachkhay/').then(response => {
    this.setState({
...
axios.get('/index.php/api/doctor').then(response => {
  ...
  axios.get('/index.php/api/customers').then(response => {
    this.setState({
      danhsachkhachhang: response.data
    })
  })  
axios.get('/index.php/api/getchinhanh').then(response => {
  ...
  render() {
    ...
    const { listkhay,listbacsi,danhsachchinhanh,danhsachkhachhang  } = this.state
...
>
<RequiredRule />
 <Lookup dataSource={danhsachkhachhang} valueExpr="ID" displayExpr="hoten" />
</Column>
<Column
caption="Số khay"
dataField="sokhay"
