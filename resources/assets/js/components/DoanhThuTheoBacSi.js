...
const columns = ['STT', 'Bác sĩ', 'Họ và tên', 'Dịch vụ', 'Tổng chi phí', 'Tiền đã thanh toán', 'Tiền còn nợ', 'Hành động'];
...
class DoanhThuTheoBacSi extends Component {
    constructor (props) {
    super(props)
    this.state = {
        khammoi: [],
...
componentDidMount(){
 document.getElementById("loadingapp").style.display = "block";
    axios.get('/index.php/api/getallkhammoitrongngaybacsi/'+localStorage.getItem('userkhid')).then(response => {
...
{/* /tabs */}
      ...
      <div className="col-sm-12">
        <div className="white-box">
          <h3 className="box-title">DOANH THU BÁC SĨ
  </h3>
    ...
    <div className="row">
      ...
      >
        ...
        <Column
         caption="Dịch vụ"
          dataField="khammoi.dichvus.ten"
          dataType="string"
        />
         <Column
         caption="Tổng tiền"
         dataField="tongchiphidieutri"
         cellRender={cellRenderRowPrice}
     
        />
        <Column
         caption="Doanh thu trong ngày"
         dataField="tongtien"
         dataType="number"
         cellRender={cellRenderRowPrice}
        />
...
function cellRenderRowChiTiet(data) {
  
  
  return(
    JSON.parse(data.value).map((bl, i) =>{
  

      if(i == JSON.parse(data.value).length - 1)
      {
        return <span>{bl.conlai.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</span>
      }
...
function cellRenderRowDoanhThu(data)
{
  return(
    JSON.parse(data.value).map((bl, i) =>{
      ...
      if(i == JSON.parse(data.value).length - 1)
      {
        return <span>{bl.saugiam.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")+" VNĐ"}</span>
      }
...
export default DoanhThuTheoBacSi;