...
class ViecGiao extends Component {
  constructor (props) {
    super(props)
    this.state = {
      listgiaoviec: '',
...
axios.get('/index.php/api/danhsachviecgiao/'+localStorage.getItem('userid')).then(response => {
    this.setState({
...
axios.get('/index.php/api/danhsachtaikhoan').then(response => {
  ...
  handleFieldChange (event) {
   ...
   onRowInserted(e) {
    console.log(e);
	e.data.nguoigiao=localStorage.getItem('userid')
  ...
  render() {
    ...
    return (
      ...
      <div className="row App" id="lichbooking">
        ...
        <h3 className="box-title">DANH SÁCH VIỆC ĐƯỢC GIAO
  </h3>
  <div className="row">
