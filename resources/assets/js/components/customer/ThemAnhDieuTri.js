...
class ThemAnhDieuTri extends Component {
    ...
    constructor(props){
        ...
        this.state = {
            ...
            iddieutri: this.props.match.params.iddt,
            dataanhdieutri: [],
            anhnguyengoc: [],
            supported_mime : [
                'image/jpeg',
    ...
    componentWillMount(){
        const scripts = [
          ...
          '/public/app_assets/fancybox/ekko-lightbox.min.js',
    ...
    uploadFiles(){
        ...
        hinhanhs.map((image) => {
            console.log(image)
            axios.post("/index.php/api/photodieutri", image).then(response => {
    ...
    selectImage(event, obj) {
        ...
        var image= document.getElementsByClassName("imganhdieutri");
    ...
    render() {
        ...
        const { iddieutri,dataanhdieutri,anhnguyengoc,images,hinhanhs,idkhachhang,idkhammoi  } = this.state
...
<nav>
 <ul>
   <li className="tab-current"><Link to={'/dieu-tri-theo-lich/'+idkhachhang+'/'+idkhammoi} className="sticon"><span>Quay lại</span></Link></li>
